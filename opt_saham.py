...
import pandas as pd
import numpy as np
from pypfopt import risk_models, expected_returns # EfficientFrontier is not used for the custom ARO
# from pypfopt.discrete_allocation import DiscreteAllocation, get_latest_prices # Not used in this combined version
...
def calculate_sharpe_ratio(weights, mean_returns, cov_matrix, risk_free_rate):
    ...
    port_return = np.sum(mean_returns * weights) # More direct calculation
    port_volatility = np.sqrt(np.dot(weights.T, np.dot(cov_matrix, weights)))
    if port_volatility == 0:
        return -np.inf # Avoid division by zero; assign a very low Sharpe ratio
...
def normalize_weights(weights):
    ...
    if total_weight == 0: # Avoid division by zero if all weights become zero
        # Return equal weights if normalization fails (e.g., all inputs were negative)
...
def aro_portfolio_optimization(df_prices, n_rabbits, n_iterations, alpha, risk_free_rate):
    """
    Performs portfolio optimization using the custom ARO-like algorithm from Code 2.
    ...
    if daily_returns.empty:
        ...
        return None, -np.inf, []
    if daily_returns.shape[0] < 2: # Need at least 2 data points for covariance
        st.warning("Data return harian tidak cukup untuk menghitung matriks kovarians.")
        return None, -np.inf, []


    mean_returns_calc = daily_returns.mean() * 252  # Annualized
    cov_matrix_calc = daily_returns.cov() * 252    # Annualized

    # Initialize rabbits (portfolio weight combinations)
    rabbits = [normalize_weights(np.random.rand(n_assets)) for _ in range(n_rabbits)]
    best_score = -np.inf
    best_weights_arr = np.array([]) # Initialize as empty numpy array
    history = []
    ...
    for iteration in range(n_iterations):
        for i in range(n_rabbits):
            ...
            # Ensure current_weights is a 1D array
            if current_weights.ndim > 1:
                current_weights = current_weights.flatten()


            random_vector = np.random.uniform(-1, 1, n_assets)
            # Ensure alpha and random_vector can be broadcasted or multiplied element-wise
            ...
            # Ensure mean_returns_calc and cov_matrix_calc align with n_assets
        ...
        history.append(best_score if best_score != -np.inf else None) # Store None if no valid score yet
    ...
    if not best_weights_arr.any(): # Check if best_weights_arr is still empty
        st.warning("Tidak ditemukan bobot optimal. Ini bisa terjadi jika semua iterasi gagal menghasilkan peningkatan.")
        return None, -np.inf, history


    # Convert best_weights array to a dictionary for compatibility with existing functions
...
def main():
    ...
    st.title("📊 OPTIMASI PORTOFOLIO dengan ARO (Heuristik)")
    ...
    if choice == "Home":
        ...
        if uploaded_file is not None:
            try:
                ...
                date_col_name = None
                # Try to find a date column by common names or as the first column
                ...
                if potential_date_cols:
                    ...
                    # If first column is unnamed index and second might be date
                ...
                if date_col_name:
                    try:
                        ...
                        # Ensure all other columns are numeric prices
                        price_cols = [col for col in df.columns if col != date_col_name]
                        for col in price_cols:
                            df[col] = pd.to_numeric(df[col], errors='coerce')
                        df = df.dropna(axis=1, how='any') # Drop columns with NaNs after conversion
                    ...
                    # Fallback: try to use the first column if no obvious date column named
                    try:
                        ...
                        # Attempt to use data as is, but select only numeric columns
        ...
        if st.session_state.data_df is not None:
            ...
            # --- Parameters for ARO ---
            st.subheader("Parameter Optimasi ARO")
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                n_rabbits = st.number_input("Jumlah Agen (Rabbits)", min_value=5, max_value=100, value=20, step=1)
            with col2:
                n_iterations = st.number_input("Jumlah Iterasi", min_value=10, max_value=500, value=50, step=10)
            with col3:
                alpha = st.slider("Faktor Pergerakan (Alpha)", min_value=0.01, max_value=1.0, value=0.2, step=0.01)
            with col4:
            ...
            **Cara Kerja (Sederhana):**
            ...
            if st.button("Lakukan Analisis Optimasi Portofolio (ARO)", key="analyze_aro_button"):
                with st.spinner("Melakukan analisis optimasi ARO... Harap tunggu."):
                    try:
                        ...
                        # Ensure all price columns are numeric, handle potential errors from upload
                        ...
                        # Drop rows with any NaN values in price columns which can occur after pct_change
            ...
            if st.session_state.analysis_done and st.session_state.optimal_weights:
                ...
                if st.session_state.sharpe_history:
                    ...
                    # Filter out None values before plotting if they exist
                ...
                try:
                    ...
                    # Ensure it's sorted for pct_change
                    ...
                    if not daily_returns_perf.empty and st.session_state.optimal_weights:
                        ...
                        # Ensure weights are in the correct order and format (numpy array)
                        ...
                        sharpe_ratio_perf = st.session_state.best_sharpe_aro # Use the one from optimization
